...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch both the user profile and their projects simultaneously
    ...
    <div className="flex h-screen bg-[#F5F5F7] font-sans text-[#1D1D1F] overflow-hidden selection:bg-black selection:text-white">
      ...
      <aside className="w-65 flex flex-col pt-6 pb-4 px-4">
        <div className="flex items-center px-2 mb-8">
          <div className="h-8 w-8 bg-black rounded-lg flex items-center justify-center mr-3 shadow-sm">
            <span className="text-white font-extrabold text-xs leading-none tracking-tighter">ADS</span>
          </div>
        ...
        <header className="flex justify-between items-center mb-6 px-2">
          ...
          <div className="flex items-center ml-6 relative">
            ...
            <div 
              ...
              className="h-9 w-9 bg-black rounded-full flex items-center justify-center text-white text-xs font-bold cursor-pointer hover:bg-gray-800 transition-colors shadow-sm uppercase"
            >
            ...
            {isProfileOpen && user && (
              <div className="absolute top-12 right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-[#E5E5EA] py-6 px-6 z-50 flex flex-col items-center">
                <div className="h-16 w-16 bg-black rounded-full flex items-center justify-center text-white text-xl font-bold mb-3 uppercase">
                  {getInitials(user.username)}
                </div>
                {/* Dynamic Username and Email */}
                <h3 className="text-lg font-semibold text-[#1D1D1F] truncate w-full text-center">{user.username}</h3>
                <span className="text-xs text-gray-500 font-medium mb-1 truncate w-full text-center">{user.email}</span>
                <span className="text-xs text-indigo-600 font-bold mb-6">Data Scientist</span>
                
                <button 
                  onClick={() => { localStorage.removeItem('token'); navigate('/auth'); }}
                  className="w-full py-2.5 bg-[#F5F5F7] hover:bg-[#E5E5EA] text-[#1D1D1F] font-semibold text-sm rounded-xl transition-colors cursor-pointer"
                >
                  Sign Out
                </button>
              </div>
          ...
          <div className="px-8 py-5 border-b border-[#E5E5EA] flex justify-between items-center">
            ...
            <div className="flex bg-[#F5F5F7] p-1 rounded-lg border border-[#E5E5EA]">
              <button 
                ...
                className={`p-1.5 rounded-md transition-all cursor-pointer ${viewMode === 'grid' ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-black'}`}
              >
              ...
              <button 
                ...
                className={`p-1.5 rounded-md transition-all cursor-pointer ${viewMode === 'table' ? 'bg-white shadow-sm text-black' : 'text-gray-500 hover:text-black'}`}
              >
                    ...
                    >
                      ...
                      <td className="py-4">
                        <div className="flex items-center space-x-2">
                          <div className="h-5 w-5 rounded-full bg-black text-white text-[9px] font-bold flex items-center justify-center uppercase">{getInitials(user?.username)}</div>
